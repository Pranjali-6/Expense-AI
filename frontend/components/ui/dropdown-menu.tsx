...
>(({ className, sideOffset = 6, ...props }, ref) => (
  <DropdownMenuPrimitive.Portal>
    <DropdownMenuPrimitive.Content
      ...
      className={cn(
        "z-50 min-w-56 overflow-hidden rounded-lg border border-border bg-surface p-1 elevation-2",
        className,
      )}
      {...props}
    />
...
>(({ className, tone = "default", ...props }, ref) => (
  <DropdownMenuPrimitive.Item
    ...
    className={cn(
      ...
      tone === "danger" ? "text-error-text focus:bg-error-subtle" : "text-foreground",
      className,
    )}
    {...props}
  />
